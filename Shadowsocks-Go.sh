...
CONF="/etc/shadowsocks-go/shadowsocks.conf"
...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/xOS/Shadowsocks-Go/master/service/centos" -O /etc/init.d/Shadowsocks-Go; then
		...
		chkconfig Shadowsocks-Go on
	else
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/xOS/Shadowsocks-Go/master/service/debian" -O /etc/init.d/Shadowsocks-Go; then
			echo -e "${Error} Shadowsocks 服务管理脚本下载失败 !"
			rm -rf "${FOLDER}"
			exit 1
		fi
...
Restart(){
	check_installed_status
	check_pid
	[[ ! -z ${PID} ]] && /etc/init.d/Shadowsocks-Go stop
	/etc/init.d/Shadowsocks-Go start
	#sleep 1s
	check_pid
	[[ ! -z ${PID} ]] && View
...
Uninstall(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		if [[ ! -z $(crontab -l | grep "Shadowsocks-Go.sh monitor") ]]; then
			crontab_monitor_cron_stop
		fi
		rm -rf "${FOLDER}"
		if [[ ${release} = "centos" ]]; then
			chkconfig --del Shadowsocks-Go
		else
			update-rc.d -f Shadowsocks-Go remove
		fi
		rm -rf "/etc/init.d/Shadowsocks-Go"
...
Set_crontab_monitor(){
	check_crontab_installed_status
	crontab_monitor_status=$(crontab -l|grep "Shadowsocks-Go.sh monitor")
...
crontab_monitor_cron_start(){
	...
	sed -i "/Shadowsocks-Go.sh monitor/d" "$file_1/crontab.bak"
	echo -e "\n* * * * * /bin/bash $file_1/Shadowsocks-Go.sh monitor" >> "$file_1/crontab.bak"
	crontab "$file_1/crontab.bak"
	rm -r "$file_1/crontab.bak"
	cron_config=$(crontab -l | grep "Shadowsocks-Go.sh monitor")
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/xOS/Shadowsocks-Go/master/Shadowsocks-Go.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/Shadowsocks-Go" ]]; then
		rm -rf /etc/init.d/Shadowsocks-Go
		Service
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/xOS/Shadowsocks-Go/master/Shadowsocks-Go.sh" && chmod +x Shadowsocks-Go.sh
